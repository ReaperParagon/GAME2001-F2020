...
public:
	...
	void push(T key)
	{
		// Puch to the back of the array (m_heap)
		...
		T temp = m_heap[index];					// Store a copy of the last elements in temp
	...
	// Pop works top down
	// Remove the top elements and replace it with the last element
	...
	void pop()
	{
		...
		if (m_heap.size() > 0)
		{
			// Restructuring the heap so that the largest item is at the top
			...
			while (index < (int)m_heap.size() / 2)
			{
				...
				// When my "temp" is greater than what my currentIndex is pointing to
