...
public:
	Graph(int numVerts) : m_maxVerts(numVerts), m_adjMatrix(NULL), m_vertVisits(NULL), m_vertices(NULL)
	{
		...
		m_adjMatrix = new char* [m_maxVerts];	// Instantiates 2D array as a whole
	...
	bool push(T node)
	{
		...
		// We do have enough space to push a new vertex
	...
	void attachEdge(int index1, int index2)
	{
		...
		m_adjMatrix[index1][index2] = 1;		// [c][e] = 1
		m_adjMatrix[index2][index1] = 1;		// [e][c] = 1
	}
	...
	// Stack gets popped back untill another path is found
	bool DepthFirstSearch(int startIndex, int endIndex)
	{
		...
		// Cheating - Know that we're going to use int instead of class T
